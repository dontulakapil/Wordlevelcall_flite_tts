...
// defination of word level call back handler 'OnWordCompletedListen'
class OnWordCompletedListener
{
public:
virtual OnWordCompletedListener *onWordCompleted(int startPosition);
virtual OnWordCompletedListener *onDone();	
};
...
struct android_tts_engine_funcs_t {
    ...
    android_tts_result_t (*shutdown)
            (void *engine);
   // set word level call back on
   android_tts_result_t (*setOnWordCompletedListener)
            (void *engine,
		OnWordCompletedListener *listener);
